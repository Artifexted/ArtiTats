...
const validateUserRegister = (req, res, next) => {
    ...
    if (!name.trim()) {
        throw new customError_1.CustomErrors(400, "Name is required");
    }
    else if (!/^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]+$/.test(name)) {
        throw new customError_1.CustomErrors(400, "Name must be a valid name.");
    }
    if (!email.trim()) {
        throw new customError_1.CustomErrors(400, "Email is required");
    }
    else if (!/\S+@\S+\.\S+/.test(email)) {
        throw new customError_1.CustomErrors(400, "Email is invalid");
    }
    if (!birthdate) {
        throw new customError_1.CustomErrors(400, "Birthdate is required");
    }
    else {
        ...
        if (age < 18 || (age === 18 && (monthDiff < 0 || (monthDiff === 0 && dayDiff < 0)))) {
            throw new customError_1.CustomErrors(400, "User must be at least 18 years old");
        }
    }
    if (!nDni) {
        throw new customError_1.CustomErrors(400, "nDni is required");
    }
    else if (isNaN(Number(nDni))) {
        throw new customError_1.CustomErrors(400, "nDni must be a number");
    }
